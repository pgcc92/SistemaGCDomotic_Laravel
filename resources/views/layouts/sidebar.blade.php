...
<div x-show="sidebarOpen" class="relative z-50 lg:hidden" x-cloak>
    ...
    <div x-show="sidebarOpen"
         ...
         class="fixed inset-y-0 left-0 flex w-80 max-w-[85vw] flex-col border-r border-slate-200 bg-white p-4 shadow-2xl dark:border-slate-800 dark:bg-slate-950">
        <div class="flex items-center justify-between">
            <a href="{{ route('dashboard') }}" class="flex items-center gap-2">
                ...
                <div class="leading-tight">
                    <div class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $branding->sidebarName ?? $branding->systemName }}</div>
                    <div class="text-xs text-slate-500 dark:text-slate-400">Dashboard</div>
                </div>
            </a>
            <button class="rounded-md p-2 hover:bg-slate-100 dark:hover:bg-slate-800" @click="sidebarOpen = false" aria-label="Close sidebar">
...
<!-- Desktop sidebar -->
<aside class="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
    ...
    <div class="border-t border-slate-200 p-4 text-xs text-slate-500 dark:border-slate-800 dark:text-slate-400">
        <div class="flex items-center justify-between">
            ...
            <span class="text-slate-400 dark:text-slate-500">v0</span>
        </div>
    </div>
</aside>
